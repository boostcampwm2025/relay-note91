...
- 07/22 수행 결과
    ...
    실제 RAM에서 사용 가능한 주소 범위
    하드웨어에 의해 결정됨
    3. 페이지 (Page)
    가상 메모리를 고정 크기로 나눈 단위 (일반적으로 4KB)
    메모리 관리의 최소 단위
    4. 프레임 (Frame)
    물리 메모리를 페이지와 같은 크기로 나눈 단위
    페이지가 실제로 저장되는 물리적 공간
    5. 페이지 테이블 (Page Table)
    가상 주소를 물리 주소로 변환하는 매핑 정보를 저장
    각 프로세스마다 고유한 페이지 테이블 보유
    6. MMU (Memory Management Unit)
    하드웨어 구성 요소로 주소 변환을 수행
    CPU와 메모리 사이에서 가상 주소를 물리 주소로 변환
    가상 메모리의 동작 원리
    1. 주소 변환 과정
    ...
    물리 주소 계산: 물리 프레임 번호 + 오프셋으로 최종 물리 주소 계산
    메모리 접근: 계산된 물리 주소로 실제 메모리에 접근
    2. 페이지 폴트 (Page Fault) 처리
    페이지 부재 발생: 접근하려는 페이지가 물리 메모리에 없음
    인터럽트 발생: MMU가 페이지 폴트 인터럽트를 CPU에 전달
    운영체제 개입: 페이지 폴트 핸들러가 실행됨
    페이지 로딩: 보조 저장장치(디스크)에서 해당 페이지를 물리 메모리로 로드
    페이지 테이블 업데이트: 새로운 매핑 정보로 페이지 테이블 갱신
    프로세스 재개: 원래 명령어부터 다시 실행
    운영체제에서의 동작 예시
    예시 상황
    물리 메모리: 1GB
    프로세스 A: 가상 주소 공간 2GB 사용
    프로세스 B: 가상 주소 공간 1.5GB 사용
    동작 과정
    프로세스 생성 시:

    운영체제가 각 프로세스에 독립적인 페이지 테이블 생성
    초기에는 대부분의 페이지가 물리 메모리에 로드되지 않음
    메모리 접근 시:

    프로세스 A가 가상 주소 0x12345678에 접근
    MMU가 페이지 번호(0x12345)와 오프셋(0x678)으로 분할
    페이지 테이블에서 페이지 0x12345가 물리 프레임 0x200에 매핑됨을 확인
    최종 물리 주소: 0x200678로 접근
    페이지 폴트 발생 시:

    프로세스 B가 가상 주소 0x87654321에 접근했지만 해당 페이지가 메모리에 없음
    페이지 폴트 인터럽트 발생
    운영체제가 디스크에서 해당 페이지를 읽어와 빈 물리 프레임에 로드
    만약 빈 프레임이 없다면, 기존 페이지를 디스크로 스왑 아웃 후 새 페이지 로드
    메모리 보호:

    프로세스 A는 자신의 페이지 테이블을 통해서만 메모리 접근 가능
    프로세스 B의 메모리 영역에 접근하려 하면 접근 권한 위반으로 세그멘테이션 폴트 발생
    ```
